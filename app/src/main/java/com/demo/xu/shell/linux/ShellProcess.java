...
public class ShellProcess {
    ...
    public synchronized void execute(String linuxCommand,ExecuteCallback callback){
        Log.i(TAG,String.format("execute command ' %s ' on thread %s",linuxCommand,Thread.currentThread().getId()));

        //若已经关闭，抛出异常
        if(isClose()){
            throw new IllegalStateException("process is already shut down");
        }
